...
inline void MultiBoxTargetForward(const Tensor<gpu, 2, DType> &loc_target,
  ...
  hipLaunchKernel(FindBestMatches, dim3(num_batches), dim3(num_threads), 0, 0, best_matches, gt_flags, anchor_flags, overlaps, num_anchors, num_labels);
  MULTIBOX_TARGET_CUDA_CHECK(hipPeekAtLastError());
  ...
  if (negative_mining_ratio > 0) {
    ...
    hipLaunchKernel(UseAllNegatives, dim3(num_blocks), dim3(num_threads), 0, 0, anchor_flags, num_batches * num_anchors);
    MULTIBOX_TARGET_CUDA_CHECK(hipPeekAtLastError());
  }
