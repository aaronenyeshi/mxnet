...
ReduceImplConfig<ndim> ConfigureReduceImpl(const TBlob& small, const TBlob& big, const TBlob* lhs,
  ...
  if (config.M == 1) {
    config.kernel_1. hipBlockDim_x = kMaxThreadsPerBlock;
    config.kernel_1.hipGridDim_x  = std::min((unsigned int)kBaseGridNum,
      (config.N + config.kernel_1. hipBlockDim_x - 1)/config.kernel_1. hipBlockDim_x);
  } else {
    ...
    config.kernel_1. hipBlockDim_x = 0;
    config.kernel_1.hipBlockDim_y = 0;

    if (config.kernel_1.do_transpose) {
      // Fastest thread ID goes through M
      // Loop over N has step size config.kernel_1.hipBlockDim_y
      if (config.N < 8) {
        config.kernel_1.hipBlockDim_y = 1;
      } else if (config.N < 256) {
        config.kernel_1.hipBlockDim_y = 4;
      } else {
        if (config.M < 8) {
          config.kernel_1. hipBlockDim_x = 1;
        } else if (config.M < 256) {
          config.kernel_1. hipBlockDim_x = 4;
        } else {
          config.kernel_1. hipBlockDim_x = config.warpSize;
        }
      }
    } else {
      // Fastest thread ID goes through N
      // Loop over M has step size config.kernel_1.hipBlockDim_y
      if (config.M < 8) {
        config.kernel_1.hipBlockDim_y = 1;
      } else if (config.M < 256) {
        config.kernel_1.hipBlockDim_y = 4;
    ...
    if (config.kernel_1. hipBlockDim_x == 0 && config.kernel_1.hipBlockDim_y == 0) {
      LOG(FATAL) << "Unable to set blockDim";
    } else if (config.kernel_1. hipBlockDim_x == 0) {
      config.kernel_1. hipBlockDim_x = nthread_reduce / config.kernel_1.hipBlockDim_y;
    } else if (config.kernel_1.hipBlockDim_y == 0) {
      config.kernel_1.hipBlockDim_y = nthread_reduce / config.kernel_1. hipBlockDim_x;
    }

    if (config.kernel_1.do_transpose) {
      // Fastest thread ID goes through M
      config.kernel_1.hipGridDim_x  = std::min((unsigned int)kBaseGridNum,
        ceil_idiv<unsigned int>(config.N, config.kernel_1.hipBlockDim_y));
      config.kernel_1.hipGridDim_y = std::min(kBaseGridNum, config.Mnext);
      int by = config.kernel_1.hipBlockDim_y;
      if (config.kernel_1.hipBlockDim_y % config.warpSize == 0) {
        // Fix shared memory bank conflict
        by++;
      }
      config.kernel_1.shMemSize = (config.kernel_1. hipBlockDim_x > 1) ?
        config.kernel_1. hipBlockDim_x*by*sizeof(DType) : 0;
      // Maximum number of times we want TB to loop in M
      // Max size of M-block each TB can handle
      int maxMblock = config.kernel_1. hipBlockDim_x*config.maxLoopPerTB;
      config.Mnext = (config.M + maxMblock - 1) / maxMblock;
    } else {
      // Fastest thread ID goes through N
      config.kernel_1.hipGridDim_x  = std::min((unsigned int)kBaseGridNum,
        ceil_idiv<unsigned int>(config.N, config.kernel_1. hipBlockDim_x));
      config.kernel_1.hipGridDim_y = std::min(kBaseGridNum, config.Mnext);
      config.kernel_1.shMemSize = (config.kernel_1.hipBlockDim_y > 1) ?
        config.kernel_1. hipBlockDim_x*config.kernel_1.hipBlockDim_y*sizeof(DType) : 0;
      // Maximum number of times we want TB to loop in M
      // Max size of M-block each TB can handle
      int maxMblock = config.kernel_1.hipBlockDim_y*config.maxLoopPerTB;
      config.Mnext = (config.M + maxMblock - 1) / maxMblock;
    }
...
void ReduceImpl(hipStream_t stream, const TBlob& small, const OpReqType req,
  ...
  if (config.M == 1) {
    ...
    const int by = (config.kernel_1.do_transpose) ?
      config.kernel_1. hipBlockDim_x : config.kernel_1.hipBlockDim_y;
